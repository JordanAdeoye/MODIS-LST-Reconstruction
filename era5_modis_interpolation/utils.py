...
def Era_interpolation(input_folder, era5_file_path, output_folder, view_time='Day', n_jobs=8):
    ...
    for fil in filenames:  # For testing, one file
        ...
        skt = data["skt"].sel(time=slice(time_min, time_max))
        ...
        output_path = os.path.join(output_folder, fil.replace(".hdf", "_skt_interp.nc"))
        ...
        with Dataset(output_path, "w", format="NETCDF4") as nc_file:
            ...
            # Data variable
            skt_var = nc_file.createVariable("skt_interp", "f4", ("time", "lat", "lon",))
            ...
            skt_var.units = "K"
            skt_var.long_name = "Interpolated ERA5 skin temperature"
