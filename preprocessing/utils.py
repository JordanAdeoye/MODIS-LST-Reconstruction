...
def inverse_maps(H,V,size):
    """
    Converts from MODIS Sinusoidal projection to geographic coordinates (lat/lon)
    """
...
def LST_extraction(input_folder, var, qc_var, varo, output_folder):
    """
    Performs Quality Assurance assessment on each pixel using MODIS QC flags.
    Pixels that fail quality criteria OR have 0 values are replaced with NaN.
    """
